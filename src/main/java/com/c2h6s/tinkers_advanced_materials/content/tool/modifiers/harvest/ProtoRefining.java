...
public class ProtoRefining extends EtSTBaseModifier implements EnchantmentModifierHook, BlockBreakModifierHook {
    ...
    public static int getBonus(IToolStackView tool,ModifierEntry entry){
        ModDataNBT nbt = tool.getPersistentData();
        return Mth.clamp(nbt.getInt(KEY_REFINE) / TiAcMeConfig.COMMON.PROTO_REFINING_TIMES_REQUIRED.get(),0,entry.getLevel()*TiAcMeConfig.COMMON.PROTO_REFINING_BONUS_LEVEL.get());
    }
    public static void addBonus(IToolStackView tool,ModifierEntry entry){
        ModDataNBT nbt = tool.getPersistentData();
        nbt.putInt(KEY_REFINE,Math.min(nbt.getInt(KEY_REFINE)+entry.getLevel(),entry.getLevel()*TiAcMeConfig.COMMON.PROTO_REFINING_TIMES_REQUIRED.get()*2*TiAcMeConfig.COMMON.PROTO_REFINING_BONUS_LEVEL.get()));
    }

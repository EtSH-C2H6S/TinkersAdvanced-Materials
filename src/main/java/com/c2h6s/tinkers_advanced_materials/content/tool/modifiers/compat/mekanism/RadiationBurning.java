...
public class RadiationBurning extends EtSTBaseModifier {
    ...
    public float onGetMeleeDamage(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float baseDamage, float damage) {
        if (context.getTarget() instanceof LivingEntity living){
            ...
            if (optional.isPresent()){
                ...
                float bonus = (float) Math.min(TiAcMeConfig.COMMON.IRRADIUM_MAX_BONUS.get(),cap.getRadiation() * TiAcMeConfig.COMMON.IRRADIUM_BONUS_PER_Sv.get());
                if (bonus>0.001){
    ...
    public void afterMeleeHit(IToolStackView tool, ModifierEntry modifier, ToolAttackContext context, float damageDealt) {
        if (context.getTarget() instanceof LivingEntity living&&context.isFullyCharged()){
            RADIATION_MANAGER.radiate(living,modifier.getLevel()*TiAcMeConfig.COMMON.IRRADIUM_RADIATION_INFLICT.get());
        }
    }

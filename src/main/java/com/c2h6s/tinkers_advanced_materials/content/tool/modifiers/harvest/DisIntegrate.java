...
public class DisIntegrate extends EtSTBaseModifier implements BlockBreakModifierHook , BreakSpeedModifierHook {
    ...
    public void modifierOnInventoryTick(IToolStackView tool, ModifierEntry modifier, Level world, LivingEntity holder, int itemSlot, boolean isSelected, boolean isCorrectSlot, ItemStack stack) {
        if (tool.getPersistentData().getInt(KEY_DISINTEGRATE)>0&&!world.isClientSide&&world.getGameTime()%20==0){
            if ((int) (world.getGameTime()%Integer.MAX_VALUE) - tool.getPersistentData().getInt(KEY_LAST_TICK_1)>=20*TiAcMeConfig.COMMON.DISINTEGRATE_EACH_DECREASE.get()) tool.getPersistentData().remove(KEY_DISINTEGRATE);
        }
    }

    @Override
    public void afterBlockBreak(IToolStackView tool, ModifierEntry modifierEntry, ToolHarvestContext toolHarvestContext) {
        if (tool.getPersistentData().getInt(KEY_DISINTEGRATE)<TiAcMeConfig.COMMON.DISINTEGRATE_MAX_BONUS.get()) tool.getPersistentData().putInt(KEY_DISINTEGRATE,tool.getPersistentData().getInt(KEY_DISINTEGRATE)+TiAcMeConfig.COMMON.DISINTEGRATE_EACH_DECREASE.get()*modifierEntry.getLevel());
